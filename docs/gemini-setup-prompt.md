# Gemini Setup Prompt for Spectacular

Copy everything below this line and paste it into Gemini CLI in your project.
...
Add a "Specification" section to this project's `GEMINI.md` at the repository root (create the file if it doesn't exist). Gemini CLI reads `GEMINI.md` files from the current directory up to the project root and down into subdirectories.
...
Note: Gemini uses `GEMINI.md` (not `CLAUDE.md` or `AGENTS.md`). You can also use the `@path/to/file.md` import syntax within `GEMINI.md` to modularize instructions. For example, you could import the language spec directly:

```markdown
@node_modules/spectacular-lang/language-spec.md
```

This inlines the full language spec into Gemini's context automatically.
